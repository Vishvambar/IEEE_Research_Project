def generate_cascade_svg():
    ...
    svg.append('<filter id="shadow" x="-5%" y="-5%" width="110%" height="110%"><feDropShadow dx="2" dy="4" stdDeviation="3" flood-opacity="0.1"/></filter>')
    svg.append('</defs>')
    ...
    def draw_disease_node(x, y, title, subtype, bullets, color_grad):
        svg_chunk = []
        # Main Node Box
        svg_chunk.append(f'<rect x="{x}" y="{y}" width="340" height="130" rx="12" ry="12" fill="url(#{color_grad})" stroke="#37474F" stroke-width="2" filter="url(#shadow)"/>')
        # Subtype Tag
    ...
    def draw_link_box(x, y, header, lines):
        svg_chunk = []
        svg_chunk.append(f'<rect x="{x}" y="{y}" width="360" height="100" rx="8" ry="8" fill="url(#grad_link)" stroke="#FBC02D" stroke-width="1.5" filter="url(#shadow)"/>')
    ...
    svg.append(draw_disease_node(50, 100, "1. Diabetes", "Chronic Metabolic", 
                                 ["High blood sugar damages blood vessels.", "Causes arterial stiffening & microvascular injury."], "grad_chronic"))
    
    svg.append(draw_disease_node(50, 290, "2. Congestive Heart Failure", "Chronic Cardiovascular", 
                                 ["Heart cannot pump blood efficiently.", "Causes systemic congestion & fluid retention."], "grad_chronic"))
